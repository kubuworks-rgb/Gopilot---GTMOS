...
def bootstrap(principal: Current) -> dict[str, object]:
    ...
    products = [p for p in repository.products.values() if p.workspace_id == principal.workspace_id]
    runs = [r for r in repository.runs.values() if r.workspace_id == principal.workspace_id]
    return {
        "mode": "fixture",
        "demo_data": True,
        "workspace": workspace,
        ...
        "accounts": [a for a in repository.accounts.values() if a.workspace_id == principal.workspace_id],
        "approval_count": sum(1 for b in repository.briefs.values() if b.account.workspace_id == principal.workspace_id and b.campaign.status == "draft"),
    }
...
def create_product(payload: ProductProfileInput, principal: Current) -> ProductProfile:
    product = repository.create_product(principal.workspace_id, payload.company_name, payload.website, payload.product, payload.target_market)
    repository.record(principal.workspace_id, principal.user_id, "product_confirmed", "product", product.id)
    return product


@router.post("/research-runs", response_model=ResearchRun, status_code=202)
def create_research(product_id: str, background_tasks: BackgroundTasks, principal: Current) -> ResearchRun:
    try:
    ...
    repository.record(principal.workspace_id, principal.user_id, "research_run_started", "research_run", run.id)
    return run
...
def select_icp(icp_id: str, principal: Current) -> ICP:
    try:
        ...
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    repository.record(principal.workspace_id, principal.user_id, "icp_selected", "icp", icp_id)
    return item
...
def list_accounts(principal: Current, min_priority: int = 0) -> list[Account]:
    return sorted(
        [a for a in repository.accounts.values() if a.workspace_id == principal.workspace_id and a.scores.priority >= min_priority],
        key=lambda item: item.scores.priority,
        reverse=True,
    )


@router.get("/accounts/{account_id}/opportunity-brief", response_model=AccountOpportunityBrief)
def get_brief(account_id: str, principal: Current) -> AccountOpportunityBrief:
    ...
    repository.record(principal.workspace_id, principal.user_id, "opportunity_brief_viewed", "account", account_id)
    return brief


@router.patch("/campaigns/{campaign_id}", response_model=CampaignDraft)
def update_campaign(campaign_id: str, payload: CampaignUpdate, principal: Current) -> CampaignDraft:
    brief = next((b for b in repository.briefs.values() if b.campaign.id == campaign_id and b.account.workspace_id == principal.workspace_id), None)
    if brief is None:
    ...
    repository.record(principal.workspace_id, principal.user_id, event, "campaign", campaign.id)
    return campaign
...
def export_accounts(principal: Current) -> Response:
    approved = [b for b in repository.briefs.values() if b.account.workspace_id == principal.workspace_id and b.campaign.status == "approved"]
    output = io.StringIO(newline="")
    writer = csv.writer(output)
    writer.writerow(["company", "domain", "fit", "intent", "confidence", "priority", "top_signal", "recommended_action"])
    for brief in approved:
        account = brief.account
        writer.writerow([_csv_safe(account.name), _csv_safe(account.domain), account.scores.fit.score, account.scores.intent.score, account.scores.confidence.score, account.scores.priority, _csv_safe(account.top_signal), _csv_safe(account.recommended_action)])
        repository.record(principal.workspace_id, principal.user_id, "account_exported", "account", account.id)
    return Response(content=output.getvalue(), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=approved-accounts.csv"})


@router.get("/audit", response_model=list[AuditEvent])
def audit(principal: Current) -> list[AuditEvent]:
    return [event for event in repository.audit if event.workspace_id == principal.workspace_id]
