...
function FeedbackButtons({ targetType, targetId }: { targetType: "account" | "signal" | "finding" | "brief"; targetId: string }) {
  ...
  async function send(rating: "GOOD_ACCOUNT" | "NEEDS_REVIEW") {
    const reason = rating === "NEEDS_REVIEW" ? window.prompt("Why does this candidate need review? (for example: wrong domain, weak fit, stale signal)")?.trim() : undefined;
    if (rating === "NEEDS_REVIEW" && !reason) {
      setStatus("Reason required");
      return;
    }
    setStatus("Saving…");
    try {
      await api.feedback({ target_type: targetType, target_id: targetId, rating, reason });
