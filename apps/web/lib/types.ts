...
export interface ResearchRun { id: string; status: string; current_stage: string; searches_used: number; documents_used: number; findings: Finding[]; product_mode: ProductMode }
...
export interface Bootstrap { mode: string; demo_data: boolean; product_mode: ProductMode; mode_availability: ModeAvailability; provider_message: string; workspace: Workspace; product: Product | null; research_run: ResearchRun | null; icps: ICP[]; accounts: Account[]; approval_count: number; capabilities?: Capability[] }
...
export interface Signal { id: string; signal_type: string; description: string; observed_at: string; strength: number; evidence_ids: string[]; entity_match_score: number; event_confidence: number; relevance: number; source_role: string }
export interface Campaign { id: string; subject: string; body: string; status: "draft" | "approved" | "rejected"; evidence_ids: string[] }
export interface Brief { account: Account; why_it_fits: EvidenceClaim[]; why_now: EvidenceClaim[]; pain_hypotheses: EvidenceClaim[]; recommended_problem: string; recommended_offer: string; recommended_action: string; risks: string[]; evidence: Evidence[]; sources: Source[]; signals: Signal[]; verified_facts: EvidenceClaim[]; unknowns: string[]; research_candidate: boolean; brief_state: BriefState; verified_identity: Record<string, unknown>; verified_icp_facts: EvidenceClaim[]; icp_mismatches: string[]; unknown_icp_facts: string[]; current_signals: Signal[]; rejected_or_ambiguous_evidence: Record<string, unknown>[]; hypotheses: EvidenceClaim[]; reason_not_to_target?: string; next_research_step?: string; campaign: Campaign; generated_at: string }
